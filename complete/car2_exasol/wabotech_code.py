...
import time
from onycom.logutil import LogUtil
from onycom.configutil import ConfigUtil
from onycom.dbutil import DBUtil

class WabotchCode:
    
    def __init__(self,dbUtil: DBUtil, logUtil: LogUtil):
        self.dbUtil = dbUtil
        self.logUtil = logUtil
    
    def create_table(self,data, table_nm):
        """
            data : 데이터
            table_nm : 테이블명
        """
        ...
        sql = 'create or replace table VSYSE.' + table_nm + '( \n'
    ...
    def wabotchCode(self):
        ...
        # insider db
        wd = self.dbUtil.make_db_connection()
        wz = pyexasol.connect(dsn='172.29.135.35/F99FAB2444F86051A9A467F6313FAAB48AF7C4760663430958E3B89A9DC53361:8563', user='sys', password='exasol', compression=True, schema='VSYSZ') # !!! 수정(2024.01.23)
        ...
        self.logUtil.logger.info('data load : STD_CEG_GRD_LST 종료 %d초' % (time.time() - start_time))
        

        ## 등록정보(STD_CEG_GRD_LST) 5등급만
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_CEG_GRD_LST(5등급) 시작')

        # 36.2s
        cs5_raw = wz.export_to_pandas("SELECT VIN, BSPL_STDG_CD, EXHST_GAS_GRD_CD, EXHST_GAS_CERT_NO, VHCL_ERSR_YN, MANP_MNG_NO, YRIDNW, VHCTY_CD, PURPS_CD2, FRST_REG_YMD, VHCL_FBCTN_YMD, VHCL_MNG_NO, VHRNO, EXTGAS_INSP_VLD_YMD, VHCL_OWNR_CL_CD, FUEL_CD, VHCTY_TY_CD2, MNFCTR_NM, VHCNM, VHCL_FRM, EGIN_TY, VHCTY_CL_CD, TOTL_WGHT, CRYNG_WGHT, DSPLVL, EGIN_OTPT FROM VSYSZ.STD_CEG_GRD_LST WHERE EXHST_GAS_GRD_CD = 'A0505' AND CLS_YMD = SELECT MAX(CLS_YMD) FROM VSYSZ.STD_CEG_GRD_LST;")
        cs5_raw_dict = {
            'VIN':'차대번호', 
            'BSPL_STDG_CD':'법정동코드', 
            'EXHST_GAS_GRD_CD':'배출가스등급', 
        ...
        self.logUtil.logger.info('data load : STD_CEG_GRD_LST(5등급) 종료 %d초' % (time.time() - start_time))
...
#         self.logUtil.logger.info('data load : STD_CEG_CAR_MIG 종료 %d초' % (time.time() - start_time))
        ...
        self.logUtil.logger.info('data load : STD_CEG_CAR_SRC_MIG 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_TB_JGT_HIS 시작')
        ...
        self.logUtil.logger.info('data load : STD_TB_JGT_HIS 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_TB_EET_HIS_ME 시작')
        ...
        self.logUtil.logger.info('data load : STD_TB_EET_HIS_ME 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_BJCD_INFO 시작')
        ...
        self.logUtil.logger.info('data load : STD_BJCD_INFO 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_DLM_TB_ERP_EARLY_ERASE_AEA 시작')
        ...
        self.logUtil.logger.info('data load : STD_DLM_TB_ERP_EARLY_ERASE_AEA 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_DLM_TB_ERP_EARLY_ERASE_LGV 시작')
        ...
        self.logUtil.logger.info('data load : STD_DLM_TB_ERP_EARLY_ERASE_LGV 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_DLM_TB_ERP_ATT_HIS 시작')
        ...
        self.logUtil.logger.info('data load : STD_N_IS_ISSUE_DISCLOSURE 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_N_IS_ISSUE 시작')
        ...
        self.logUtil.logger.info('data load : STD_N_IS_ISSUE 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : N_IS_PENALTY 시작')
        ...
        self.logUtil.logger.info('data load : N_IS_PENALTY 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : N_US_DISCLOSURE 시작')
        ...
        self.logUtil.logger.info('data load : N_US_DISCLOSURE 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_N_US_PENALTY 시작')
        ...
        self.logUtil.logger.info('data load : STD_N_US_PENALTY 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_BD_TB_MAPDATA 시작')
        ...
        self.logUtil.logger.info('data load : STD_BD_TB_MAPDATA 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_BD_GRD4_RESULT 시작')
        ...
        self.logUtil.logger.info('data load : STD_BD_GRD4_RESULT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data load : STD_BD_KOSIS 시작')
        ...
        self.logUtil.logger.info('data load : STD_BD_KOSIS 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_CAR_CURSTT 시작')
        ...
        # self.logUtil.logger.info('data load : STD_BD_GRD5_LMT_NOCS')
...
#         cs = carm.merge(srcr, on='제원관리번호', how='left')
        ...
        # 2m 0.5s
        cs5i = cs5m.merge(insm, on='차대번호', how='left') # !!! 수정(2024.01.23)
        ...
        self.create_table(STD_BD_GRD4_CAR_CURSTT,'STD_BD_GRD4_CAR_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_CAR_CURSTT 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_CAR_CURSTT 시작')
        ...
        self.create_table(STD_BD_GRD5_CAR_CURSTT,'STD_BD_GRD5_CAR_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD5_CAR_CURSTT 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_CAR_CURSTT_TOT 시작')
        ...
        self.create_table(STD_BD_GRD4_CAR_CURSTT_TOT,'STD_BD_GRD4_CAR_CURSTT_TOT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_CAR_CURSTT_TOT 종료 %d초' % (time.time() - start_time))
        

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_SI 시작')
        ...
        ### [출력] STD_BD_GRD4_SI
        self.create_table(STD_BD_GRD4_SI,'STD_BD_GRD4_SI')
        self.logUtil.logger.info('data export : STD_BD_GRD4_SI 종료 %d초' % (time.time() - start_time))
        
        

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_CERT_NO_RVW 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_CERT_NO_RVW,'STD_BD_DAT_GRD4_CERT_NO_RVW')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_CERT_NO_RVW 종료 %d초' % (time.time() - start_time))     
        

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_SI 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_SI,'STD_BD_DAT_GRD4_SI')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_SI 종료 %d초' % (time.time() - start_time))
...
#             else:
#                 self.logUtil.logger.info(f'오류 배인번호 : {one}')
#                 pass
...
#         self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_SI 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_ELPDSRC_CURSTT 시작')
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_ELPDSRC_CURSTT_MOD 시작')
        ...
        # 조기폐차 추가
        df5e = cs5ec.merge(elpm, on='차대번호', how='left')
        df51 = df5e[df5e['연료'] == '경유'].reset_index(drop=True)

        # 조기폐차 해당 차량 추출
        idx = df51.loc[df51['조기폐차최종승인YN'] == 'Y'].index
        df51_ey = df51.loc[idx]
        df51_en = df51.loc[list(set(df51.index) - set(idx))]

        # 기준연월 추가
        df51_ey['말소일자'] = df51_ey['말소일자'].astype('str')
        df51_ey['기준연월'] = df51_ey['말소일자'].str[:4] + '.' + df51_ey['말소일자'].str[4:6]
        df51_ey[['말소일자', '기준연월']].head()

        # 다시 병합
        df51 = pd.concat([df51_ey, df51_en], ignore_index=True)

        today_date = datetime.today().strftime("%Y%m%d")

        # 조기폐차 이상치 말소일자 제거
        df51 = df51[(df51['말소일자'] <= today_date) | (df51['말소일자'].isnull())].reset_index(drop=True)

        STD_BD_GRD5_ELPDSRC_CURSTT_MOD = df51[[
            '기준연월',
            '차대번호', 
            '법정동코드', 
            '차종', 
        ...
        today_date = datetime.today().strftime("%Y%m%d")
        STD_BD_GRD5_ELPDSRC_CURSTT_MOD['테이블생성일자'] = today_date
        STD_BD_GRD5_ELPDSRC_CURSTT_MOD = STD_BD_GRD5_ELPDSRC_CURSTT_MOD[[
            '기준연월',
            '차대번호', 
            '법정동코드', 
            '차종', 
        ...
        STD_BD_GRD5_ELPDSRC_CURSTT_MOD = STD_BD_GRD5_ELPDSRC_CURSTT_MOD.rename(columns=chc_dict)

        # 지역명 수정
        elpd_dict = {'강원도':'강원특별자치도'}
        STD_BD_GRD5_ELPDSRC_CURSTT_MOD['CTPV'] = STD_BD_GRD5_ELPDSRC_CURSTT_MOD['CTPV'].replace(elpd_dict)
        ...
        STD_BD_GRD5_ELPDSRC_CURSTT_MOD['RGN'] = STD_BD_GRD5_ELPDSRC_CURSTT_MOD['RGN'].replace(ctpv_dict)
        
        ### [출력] STD_BD_GRD5_ELPDSRC_CURSTT_MOD
        self.create_table(STD_BD_GRD5_ELPDSRC_CURSTT_MOD,'STD_BD_GRD5_ELPDSRC_CURSTT_MOD')
        self.logUtil.logger.info('data export : STD_BD_GRD5_ELPDSRC_CURSTT_MOD 종료 %d초' % (time.time() - start_time)) 
        
        # !!! 수정 끝(2024.02.02)

        # !!! 수정 시작(2023.10.27)
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_NOW_ELPDSRC_CURSTT 시작')
        ...
        self.create_table(STD_BD_GRD4_NOW_ELPDSRC_CURSTT,'STD_BD_GRD4_NOW_ELPDSRC_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_NOW_ELPDSRC_CURSTT 종료 %d초' % (time.time() - start_time))

        # !!! 수정 끝(2023.10.26)
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_MLSFC_RSLT 시작')
        ...
        self.create_table(STD_BD_GRD4_MLSFC_RSLT,'STD_BD_GRD4_MLSFC_RSLT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_MLSFC_RSLT 종료 %d초' % (time.time() - start_time))
        



        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_MLSFC 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_MLSFC,'STD_BD_DAT_GRD4_MLSFC')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_MLSFC 종료 %d초' % (time.time() - start_time))
        ...
        self.create_table(STD_BD_DAT_GRD4_DTL_INFO,'STD_BD_DAT_GRD4_DTL_INFO')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_DTL_INFO 종료 %d초' % (time.time() - start_time))
        


        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_RGN_CURSTT 시작')
...
#         base = pd.DataFrame({'연도':yr_list, '월':mth_list, '연료':fuel_list, '시도':ctpv_list, '시군구_수정':sgg_list})
        ...
        self.create_table(STD_BD_GRD4_RGN_CURSTT,'STD_BD_GRD4_RGN_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_RGN_CURSTT 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_RGN_CURSTT 시작')
        ...
        self.create_table(STD_BD_GRD4_RGN_CURSTT,'STD_BD_GRD5_RGN_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD5_RGN_CURSTT 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_RGN_CURSTT_MOD 시작')
        ...
        self.create_table(STD_BD_GRD4_RGN_CURSTT_MOD,'STD_BD_GRD4_RGN_CURSTT_MOD')
        self.logUtil.logger.info('data export : STD_BD_GRD4_RGN_CURSTT_MOD 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_RGN_CURSTT_MOD 시작')
        ...
        self.create_table(STD_BD_GRD5_RGN_CURSTT_MOD,'STD_BD_GRD5_RGN_CURSTT_MOD')
        self.logUtil.logger.info('data export : STD_BD_GRD5_RGN_CURSTT_MOD 종료 %d초' % (time.time() - start_time))
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_CAR_CURSTT 시작')
        ...
        # erea = erea.reset_index(drop=True)
        # print(erea.shape)
        # erea.columns
        ...
        self.create_table(STD_BD_DAT_GRD4_CAR_CURSTT,'STD_BD_DAT_GRD4_CAR_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_CAR_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_EXHST_GAS_MSS_TOT 시작')
        ...
        # 기준연월 추가
        # grp1['기준연월'] = '2022.12'
        grp1['기준연월'] = crtr_year + '.' + crtr_month
        ...
        ### [출력] STD_BD_GRD4_EXHST_GAS_MSS_TOT # 테이블 누적
        # self.create_table(STD_BD_GRD4_EXHST_GAS_MSS_TOT,'STD_BD_GRD4_EXHST_GAS_MSS_TOT')
        conn = self.dbUtil.make_db_connection()
        table_nm = 'STD_BD_GRD4_EXHST_GAS_MSS_TOT'
        data = STD_BD_GRD4_EXHST_GAS_MSS_TOT
        conn.import_from_pandas(data, ("VSYSE",table_nm), import_params={'columns': data.columns})
        conn.close()
        self.logUtil.logger.info('data export : STD_BD_GRD4_EXHST_GAS_MSS_TOT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_EXHST_GAS_MSS 시작')
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_EXHST_MSS_CURSTT 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_EXHST_MSS_CURSTT,'STD_BD_DAT_GRD4_EXHST_MSS_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_EXHST_MSS_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_MEVLU 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_MEVLU,'STD_BD_DAT_GRD4_MEVLU')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_MEVLU 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT_MOD 시작')
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT_MOD2 시작')
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT 시작')
        ...
        self.create_table(STD_BD_CAR_CURSTT,'STD_BD_CAR_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_REG_MKCNT 시작')
        ...
        self.create_table(STD_BD_CAR_REG_MKCNT,'STD_BD_CAR_REG_MKCNT')
        self.logUtil.logger.info('data export : STD_BD_CAR_REG_MKCNT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_PRET 시작')
        ...
        ### [출력] STD_BD_CAR_PRET
        self.create_table(STD_BD_CAR_PRET,'STD_BD_CAR_PRET')
        self.logUtil.logger.info('data export : STD_BD_CAR_PRET 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_HYBRD_CAR_PRET 시작')
        ...
        # # 음수 확인
        # self.logUtil.logger.info(df5.loc[df5['경유_하이브리드_대수'] < 0, '경유_하이브리드_대수'].shape)
        # self.logUtil.logger.info(df5.loc[df5['경유_하이브리드_예측'] < 0, '경유_하이브리드_예측'].shape)
        # self.logUtil.logger.info(df5.loc[df5['경유_하이브리드_예측_BSpline'] < 0, '경유_하이브리드_예측_BSpline'].shape)
        # self.logUtil.logger.info(df5.loc[df5['경유_하이브리드_예측_Akima'] < 0, '경유_하이브리드_예측_Akima'].shape)
        # self.logUtil.logger.info(df5.loc[df5['휘발유_하이브리드_대수'] < 0, '휘발유_하이브리드_대수'].shape)
        # self.logUtil.logger.info(df5.loc[df5['휘발유_하이브리드_예측'] < 0, '휘발유_하이브리드_예측'].shape)
        # self.logUtil.logger.info(df5.loc[df5['휘발유_하이브리드_예측_BSpline'] < 0, '휘발유_하이브리드_예측_BSpline'].shape)
        # self.logUtil.logger.info(df5.loc[df5['휘발유_하이브리드_예측_Akima'] < 0, '휘발유_하이브리드_예측_Akima'].shape)
        # self.logUtil.logger.info(df5.loc[df5['LPG_하이브리드_대수'] < 0, 'LPG_하이브리드_대수'].shape)
        # self.logUtil.logger.info(df5.loc[df5['LPG_하이브리드_예측'] < 0, 'LPG_하이브리드_예측'].shape)
        # self.logUtil.logger.info(df5.loc[df5['LPG_하이브리드_예측_BSpline'] < 0, 'LPG_하이브리드_예측_BSpline'].shape)
        # self.logUtil.logger.info(df5.loc[df5['LPG_하이브리드_예측_Akima'] < 0, 'LPG_하이브리드_예측_Akima'].shape)
        ...
        self.create_table(STD_BD_HYBRD_CAR_PRET,'STD_BD_HYBRD_CAR_PRET')
        self.logUtil.logger.info('data export : STD_BD_HYBRD_CAR_PRET 종료 %d초' % (time.time() - start_time))

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_FUEL_GRD_VHCL_CURSTT_PRET 시작')
        ...
        self.create_table(STD_BD_FUEL_GRD_VHCL_CURSTT_PRET,'STD_BD_FUEL_GRD_VHCL_CURSTT_PRET')
        self.logUtil.logger.info('data export : STD_BD_FUEL_GRD_VHCL_CURSTT_PRET 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_ECO_CAR_PRET 시작')
        ...
        # # 음수 확인
        # self.logUtil.logger.info(df7.loc[df7['전기_대수'] < 0, '전기_대수'].shape)
        # self.logUtil.logger.info(df7.loc[df7['전기_예측'] < 0, '전기_예측'].shape)
        # self.logUtil.logger.info(df7.loc[df7['전기_예측_BSpline'] < 0, '전기_예측_BSpline'].shape)
        # self.logUtil.logger.info(df7.loc[df7['전기_예측_Akima'] < 0, '전기_예측_Akima'].shape)
        # self.logUtil.logger.info(df7.loc[df7['수소_대수'] < 0, '수소_대수'].shape)
        # self.logUtil.logger.info(df7.loc[df7['수소_예측'] < 0, '수소_예측'].shape)
        # self.logUtil.logger.info(df7.loc[df7['수소_예측_BSpline'] < 0, '수소_예측_BSpline'].shape)
        # self.logUtil.logger.info(df7.loc[df7['수소_예측_Akima'] < 0, '수소_예측_Akima'].shape)
        ...
        self.create_table(STD_BD_ECO_CAR_PRET,'STD_BD_ECO_CAR_PRET')
        self.logUtil.logger.info('data export : STD_BD_ECO_CAR_PRET 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_ERSR_RSLT 시작')
        ...
        ### [출력] STD_BD_ERSR_RSLT
        self.create_table(STD_BD_ERSR_RSLT,'STD_BD_ERSR_RSLT')
        self.logUtil.logger.info('data export : STD_BD_ERSR_RSLT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD_CURSTT 시작')
        ...
        self.create_table(STD_BD_DAT_GRD_CURSTT,'STD_BD_DAT_GRD_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_FUEL_CAR_DEC 시작') 
        ...
        self.create_table(STD_BD_DAT_FUEL_CAR_DEC,'STD_BD_DAT_FUEL_CAR_DEC')
        self.logUtil.logger.info('data export : STD_BD_DAT_FUEL_CAR_DEC 종료 %d초' % (time.time() - start_time))
        
        ### !!! 수정 시작(2023.11.27)
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT_MOD_4RD_INS 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_INS_CURSTT,'STD_BD_DAT_GRD4_INS_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_INS_CURSTT 종료 %d초' % (time.time() - start_time))

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_CAR_CURSTT_MOD2_4RD_INS 시작')
        ...
        self.create_table(STD_BD_DAT_GRD4_INS_CURSTT2,'STD_BD_DAT_GRD4_INS_CURSTT2')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD4_INS_CURSTT2 종료 %d초' % (time.time() - start_time))      
        ...
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_LEM_N_MOD 시작')
        ...
        self.create_table(STD_BD_GRD5_LEM_N_MOD,'STD_BD_GRD5_LEM_N_MOD')
        self.logUtil.logger.info('data export : STD_BD_GRD5_LEM_N_MOD 종료 %d초' % (time.time() - start_time))

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_SEASON_CRDN_NOCS_CURSTT 시작')
        ...
        self.create_table(STD_BD_SEASON_CRDN_NOCS_CURSTT,'STD_BD_SEASON_CRDN_NOCS_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_SEASON_CRDN_NOCS_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_SEASON_DY_AVRG_CRDN_NOCS 시작')
        ...
        self.create_table(STD_BD_SEASON_DY_AVRG_CRDN_NOCS,'STD_BD_SEASON_DY_AVRG_CRDN_NOCS')
        self.logUtil.logger.info('data export : STD_BD_SEASON_DY_AVRG_CRDN_NOCS 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_ORDITM_DSCL_CURSTT 시작')
        ...
        self.create_table(STD_BD_ORDITM_DSCL_CURSTT,'STD_BD_ORDITM_DSCL_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_ORDITM_DSCL_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_SEASON_DSCL_RGN_CURSTT 시작')
        ...
        self.create_table(STD_BD_SEASON_DSCL_RGN_CURSTT,'STD_BD_SEASON_DSCL_RGN_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_SEASON_DSCL_RGN_CURSTT 종료 %d초' % (time.time() - start_time))
        
        ### !!! 수정 시작(2023.11.27)

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_ORDITM_RUN_LMT_CURSTT 시작')
        ...
        self.create_table(STD_BD_DAT_ORDITM_RUN_LMT_CURSTT,'STD_BD_DAT_ORDITM_RUN_LMT_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_ORDITM_RUN_LMT_CURSTT 종료 %d초' % (time.time() - start_time))

        ### !!! 수정 끝(2023.11.27)  
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD5_REDUC_BIZ 시작')
        ...
        self.create_table(STD_BD_DAT_GRD5_REDUC_BIZ,'STD_BD_DAT_GRD5_REDUC_BIZ')
        self.logUtil.logger.info('data export : STD_BD_DAT_GRD5_REDUC_BIZ 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_RUN_LMT_CURSTT 시작')
        ...
        self.create_table(STD_BD_DAT_RUN_LMT_CURSTT,'STD_BD_DAT_RUN_LMT_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_RUN_LMT_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_DS_CRRLTN_CFFCNT 시작')
        ...
        self.create_table(STD_BD_GRD4_DS_CRRLTN_CFFCNT,'STD_BD_GRD4_DS_CRRLTN_CFFCNT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_DS_CRRLTN_CFFCNT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD4_LEM_PRIO_ORD_SELCT_CURSTT 시작')
        ...
        self.create_table(STD_BD_GRD4_LEM_PRIO_ORD_SELCT_CURSTT,'STD_BD_GRD4_LEM_PRIO_ORD_SELCT_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD4_LEM_PRIO_ORD_SELCT_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_DS_CRRLTN_CFFCNT 시작')
        ...
        self.create_table(STD_BD_GRD5_DS_CRRLTN_CFFCNT,'STD_BD_GRD5_DS_CRRLTN_CFFCNT')
        self.logUtil.logger.info('data export : STD_BD_GRD5_DS_CRRLTN_CFFCNT 종료 %d초' % (time.time() - start_time))

        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_GRD5_LEM_PRIO_ORD_SELCT_CURSTT 시작')
        ...
        self.create_table(STD_BD_GRD5_LEM_PRIO_ORD_SELCT_CURSTT,'STD_BD_GRD5_LEM_PRIO_ORD_SELCT_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_GRD5_LEM_PRIO_ORD_SELCT_CURSTT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_LEM_SELCT 시작')
        ...
        self.create_table(STD_BD_DAT_LEM_SELCT,'STD_BD_DAT_LEM_SELCT')
        self.logUtil.logger.info('data export : STD_BD_DAT_LEM_SELCT 종료 %d초' % (time.time() - start_time))
        
        start_time = time.time()
        self.logUtil.logger.info('data export : STD_BD_DAT_SELCT_PNT_CURSTT 시작')
        ...
        self.create_table(STD_BD_DAT_SELCT_PNT_CURSTT,'STD_BD_DAT_SELCT_PNT_CURSTT')
        self.logUtil.logger.info('data export : STD_BD_DAT_SELCT_PNT_CURSTT 종료 %d초' % (time.time() - start_time))

        ## 3-3 code end ##################################################################
        self.logUtil.logger.info('code end')
        # code end ##################################################################
        wd.close()